export default function QuestionForm({ question, onNext }) {
    ...
    return (
        <div className="max-w-lg mx-auto text-center px-4">
            <h2 className="text-2xl sm:text-3xl font-semibold mb-8">{question.text}</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 sm:gap-2 gap-4">
                {question.options.map((option, idx) => (
                    <button
                        ...
                        className={`h-16 w-full sm:h-24 sm:w-36 rounded-xl shadow-md bg-gradient-to-r ${gradients[idx % gradients.length]} flex items-center justify-center hover:scale-105 transition-transform duration-300`}
                    >
